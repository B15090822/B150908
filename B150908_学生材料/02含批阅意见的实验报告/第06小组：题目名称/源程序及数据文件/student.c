...
void calcuMark(double m[5][2],Player pla[],int n) /*求五场比赛的最高、最低分*/
/*其中形式参数二维数组m的第一维代表五场比赛，第二维代表最高、最低*/
{
	int i,j;
	for (i=0;i<5;i++)                 /*求五场比赛的最高分*/		
	{ 
		m[i][0]=pla[0].score[i];     
		for (j=1;j<n;j++)
...
void sortStu(Player plau[],int n,int condition)  /*选择法排序，按condition条件由小到大排序*/
{
...
int deletePla(Player pla[],int n,Student s)            /*从数组中删除指定球衣号码的一个元素*/
{
	int i,j;
